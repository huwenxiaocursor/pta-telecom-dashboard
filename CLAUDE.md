...
Google News RSS / WordPress REST API / Business Recorder RSS
    ↓ is_relevant() 关键词过滤
...
news_cache.json（永久缓存，含 summary_zh、importance、dup_of）
    ↓ mark_duplicates()：确定性实体重叠去重，命中即写 dup_of 持久化（见下）
    ↓ 按日期分组（跳过 dup_of 条目）；字符串近似去重 + LLM当次语义去重（仅最近
    ↓ DEDUP_LOOKBACK_DAYS天，不持久化）；去重后当天候选覆盖的distinct来源数 ≥
...
- `is_relevant()` 关键词过滤带**地域校验**（2026-07-12 加）：命中电信/宏观关键词后，若标题明显在讲外国（`_FOREIGN`：Thai/India/China 等国家词）且完全不含巴基斯坦标识（`_PK_MARKERS`：Pakistan/SBP/PTA/Karachi 及四大运营商等）则否掉。因为 `_TELECOM_SUB` 里的宏观词（`central bank`/`inflation rate`/`interest rate`/`monetary policy`…）是全球通用的，而 BusinessRecorder 会转发 Reuters/AFP 的外国新闻（真实事故：泰国"central bank chief"通胀新闻因命中 `central bank` 混进看板）。反过来，宏观词覆盖也要留意漏收：IMF 相关只列了 `imf program/review/tranche/loan/talks/funding/disbursement/bailout` 这些**相邻短语**，`IMF approves…disbursement` 这种词被隔开的仍匹配不上——发现漏收的正当新闻时优先补 `_TELECOM_SUB` 关键词，别去松动地域校验。
- 新闻源优先级（高→低）：PTA > ProPakistani > SBP > BusinessRecorder > TechJuice，同日相似标题会去重（保留排序更靠前的一条，即重要性更高、来源优先级更高的）。PhoneWorld 已于2026-07-02因质量问题（会把过时新闻当新内容重新发布）被 Business Recorder 替换，历史缓存里的 PhoneWorld 条目保留不动。
...
- 跨源同事件去重是**三层**（都只作用于同一天候选、只处理最近 `DEDUP_LOOKBACK_DAYS`(3天)内的新闻，保留按重要性/来源排序更靠前的一条）：
  1. **`mark_duplicates()` 确定性实体重叠去重（唯一持久化的一层）**：提取标题里当天低频的显著词（人名/机构缩写等，出现文档数 ≤ `ENTITY_RARE_DF_MAX`，排除 Ufone/Telenor/merger 这类当天高频话题词），两条共享的稀有词 ≥ `ENTITY_OVERLAP_MIN_RARE`(3) 即判为同一事件，给被去掉的一条写 `dup_of`（保留条的url）存进 `news_cache.json`。**决策一旦落盘就固定**，展示时直接跳过 `dup_of` 条目——这是防止"已去重的新闻因LLM波动下次又冒出来"的关键。此层刻意保守（宁漏勿错），从不会把同话题不同角度的新闻误合并。幂等：重复运行结果完全一致，已标记的不复算。
  2. **`llm_dedup_groups()` LLM语义去重（当次展示用，不持久化）**：用DeepSeek识别措辞差异大、实体兜底抓不到的同事件重复（例如"IHC Clears..."vs"IHC Approves..."），只过滤当次展示。**刻意不持久化**：DeepSeek 即使 `temperature=0` 也非确定性，且偶尔会把同话题不同角度的新闻（如合并后的"资费上涨"vs"员工裁员"vs"暂停改名"）**过度合并**成一条——若把这种误判落盘会永久误删不同新闻，所以只让它影响单次展示、下次自我纠正。
  3. **字符串近似去重**：兜底同一天标题字面几乎相同的残留。
