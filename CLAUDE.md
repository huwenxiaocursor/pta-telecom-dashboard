...
- **九类定向排除（用户指定）**，都在 `is_relevant()` 开头、关键词匹配**之前**判定，且都带"例外"机制，避免把有价值的监管新闻一起砍掉。除燃油那类外，例外统一是"标题含 `_TELECOM_ENTITY`"：
  ...
  7. **纯事务性的机构会议/活动**（`_is_admin_event()`，2026-08-22 加）：触发案例 `SBP BSC, KP SIDB hold meeting on 'SME Value Chain Financing'`——央行下属服务公司与省级小工业委员会开了个会，通篇"讨论了""表示赞赏""期待继续合作"，没有政策也没有数据，唯一的通行证是标题里的 `sbp` 整词。**这类词极度危险，必须双重豁免**：库里 12 条含会议措辞的新闻只有这 1 条该排除，其余全是核心内容——SBP 货币政策委员会（MPC）会议 4 条（政策利率就是在那里定的）、PTA 与 Google/TikTok 的合作 2 条、PTA 5G 频谱拍卖信息备忘录 3 条。所以命中 `_ADMIN_EVENT` 后还要**同时**不含 `_TELECOM_ENTITY` 和 `_MACRO_CORE`（monetary policy / policy rate / mpc / imf / reserves / inflation / spectrum auction…）才排除。改这条规则后**必须拿库里全部会议类标题跑一遍回归**，确认只掉该掉的那条。
  8. **手机新品发布/参数评测**（`_is_phone_product()`，2026-08-22 加）：触发案例 `Honor Turbo 5G Launches With Over Three Day Battery and 6500 Nits OLED`（还是在俄罗斯发布的，与巴基斯坦无关）、`vivo V80 Lite 5G Launch Date Revealed With 10,000mAh Battery`。判据是 `_PHONE_BRANDS_WB`（整词）+ `_PRODUCT_LAUNCH`（launch/spec/camera/battery/mah/oled…）同时命中，且不含 `_PHONE_INDUSTRY`（manufactur/production/import/tax/market share/dirbs…）。
     - **品牌名必须整词匹配**，库里两条真实反例：`Opposition MNA Wants First Telecom Tower…` 和 `JazzWorld Highlights…From Opportunity…`，子串会命中 `Opp-osi-tion`、`Opp-ortunity` 里的 `oppo`。
     - **不能拿 `_TELECOM_ENTITY` 当例外**（其他几类排除都用它）：手机新品标题几乎都带 `5G`，用它豁免等于规则完全失效。所以另设 `_PHONE_INDUSTRY`——制造、进口、税收、市场份额这些产业与政策议题才是看板要的，产品参数不是。回归验证保住了 `Jam urges Samsung to set up mobile phone-making facilities`、`Pakistan Mobile Phone Production: Samsung Grows, Vivo Declines`、`Jazz Completes SDM Virtualization with Nokia`。
     - 同期给股市规则补了 `psx`/`kse` **整词**匹配（原先只列了 `psx closes`/`psx ends` 等，漏掉 `PSX Outlook…`）。标题里出现 PSX 几乎必是股市新闻，整词比逐个列举更全；电信实体例外仍在。
  9. **基层执法个案**（`_is_petty_enforcement()`，2026-08-23 加）：触发案例 `FIA Busts Copyright Ring, Arrests Man Over Illegal Internet Service`——联邦调查局抓了三个人、查获设备，刑事个案，无政策或行业信息。
     - **例外同样不能用 `_TELECOM_ENTITY`**（第 8 类也踩过这个坑）：该标题里的 `Internet` 正好在那张表中，用它豁免规则立刻失效。改判**主导方**——`_ENFORCEMENT_AUTHORITY`（pta / court / lhc / ihc / tribunal / ccp / 议会 / 监管机构）出手才有行业意义，FIA 抓人没有。
