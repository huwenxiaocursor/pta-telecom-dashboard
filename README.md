...
- 由 macOS `launchd` 调度，无需手动干预；依赖 09:00 的新闻抓取任务先完成
...
│   ├── com.cmpak.telecom-news-fetch.plist   # macOS launchd 配置（每天 09:00 PKT 触发新闻抓取）
...
| 抓取新闻 + 生成摘要 + commit/push | 每天 09:00 PKT | 本地 macOS launchd（`run_news_fetch.sh`） |
